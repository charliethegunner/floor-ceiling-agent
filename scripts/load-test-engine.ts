...
// (src/integration/engine-integration.test.ts covers the same four
// subsystems at fast, deterministic, small scale, and IS part of the
// suite). This script drives real, higher-volume concurrent load across
// real ProjectPack ingestion, real Z3/worker-pool verification, real
...
// project at ~250-500MB RSS per worker, and SandboxRunner spawns a FRESH
// worker per execution by design. Unbounded concurrency here would risk
...
const PRESSURE_TASK_EVERY_N = 4 // roughly 1-in-4 topology tasks simulates memory pressure
...
interface TaskOutcome {
  category: 'instruction' | 'topology' | 'topology-pressure' | 'sandbox'
  ok: boolean
  wallClockMs: number
...
async function runInstructionTask(index: number): Promise<void> {
  ...
  try {
    ...
    const recordedSpanLatencyMs = spans.reduce((sum, s) => {
      const latencyAttr = s.attributes.find((a) => a.key === 'latency_ms')
      return sum + (latencyAttr && 'doubleValue' in latencyAttr.value ? latencyAttr.value.doubleValue : latencyAttr && 'intValue' in latencyAttr.value ? latencyAttr.value.intValue : 0)
    }, 0)
    ...
    const gates = await pool.verify(
      {
        ...
        workspaceFiles,
        ...(isPressureTask ? { __testFakeRssBytes: 900 * 1024 * 1024 } : {}),
      },
...
function printReport(pool: WorkerPoolEvaluator, ingestMs: number, totalMs: number): void {
  ...
  console.log(`  recycledWorkerCount: ${pool.recycledWorkerCount} (expected > 0: ${TOPOLOGY_TASK_COUNT / PRESSURE_TASK_EVERY_N} pressure task(s) were injected)`)
...
async function main(): Promise<void> {
  ...
  try {
    ...
    printReport(pool, ingestMs, totalMs)

    const anyFailures = outcomes.some((o) => !o.ok)
    const recyclingObserved = pool.recycledWorkerCount > 0
    if (anyFailures) {
      console.error('LOAD TEST FAILED: at least one task did not succeed.')
      process.exitCode = 1
    } else if (!recyclingObserved) {
      console.error('LOAD TEST FAILED: no worker recycling was observed despite injected pressure tasks.')
      process.exitCode = 1
    } else {
